...
Compare to `next.config.mjs` — the shape is similar (a default export returning a config object) but the keys differ. Astro does have `redirects` and `experimental` as top-level keys, but there is no static `rewrites` map like Next.js's — call `Astro.rewrite()` at request time (in a page or middleware) instead.
