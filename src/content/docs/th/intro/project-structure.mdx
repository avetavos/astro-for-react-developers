...
เทียบกับ `next.config.mjs` — รูปแบบคล้ายกัน (default export ที่คืน config object) แต่ key ต่างกัน Astro มี `redirects` และ `experimental` เป็น top-level key จริง แต่ไม่มี static `rewrites` map แบบ Next.js — ให้เรียก `Astro.rewrite()` ตอน request แทน (ในหน้า page หรือใน middleware)
